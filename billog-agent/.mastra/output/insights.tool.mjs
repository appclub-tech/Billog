...
import { z } from 'zod';
import { d as detectItemType, s as searchSimilarItems, g as getPerishableWindow } from './expense-item-vector.mjs';
...
Returns matches sorted by date (most recent first).`,
  ...
  execute: async (input, ctx) => {
    ...
    console.log(`[Insights] \u{1F50D} search-similar-purchases: "${input.query}" (${input.lookbackDays} days)`);
    if (!sourceChannelId) {
...
Call this when user records a new expense to check for duplicates.`,
  ...
  execute: async (input, ctx) => {
    const reqCtx = ctx?.requestContext;
    const sourceChannelId = reqCtx?.get("sourceChannelId");
    console.log(`[Insights] \u{1F504} check-duplicate-purchase: ${input.items.map((i) => i.name).join(", ")}`);
    if (!sourceChannelId) {
      return {
        hasDuplicates: false,
        duplicates: [],
...
export { checkDuplicatePurchaseTool as c, detectItemTypeTool as d, getPerishableWindowTool as g, searchSimilarPurchasesTool as s };
