 ...
 * Insights Tools
 *
 * Tools for the Insights Agent to search purchase history
 * and provide shopping intelligence.
 */
...
/**
 * Search Similar Purchases Tool
 *
 * Semantic search for similar items in purchase history.
 * Works across languages: "banana" matches "กล้วย"
 */
export const searchSimilarPurchasesTool = createTool({
...
Returns matches sorted by date (most recent first).`,
  ...
  execute: async (input, ctx) => {
    ...
    console.log(`[Insights] 🔍 search-similar-purchases: "${input.query}" (${input.lookbackDays} days)`);
    ...
    return {
      ...
      message: lastPurchase
        ? `Found ${lastPurchase.name} purchased ${lastPurchase.daysSince} days ago (${lastPurchase.quantity} ${lastPurchase.unit || 'units'})`
...
/**
 * Get Perishable Window Tool
 *
 * Get the typical freshness window for an item type.
 * Used to determine if a warning is relevant.
 */
export const getPerishableWindowTool = createTool({
...
/**
 * Detect Item Type Tool
 *
 * Detect the perishable type from an item name.
 */
export const detectItemTypeTool = createTool({
  id: 'detect-item-type',
...
/**
 * Check Duplicate Purchase Tool
 *
 * Combined tool that searches and checks perishable window.
 * Returns advisory message if duplicate found within freshness window.
 */
export const checkDuplicatePurchaseTool = createTool({
...
Call this when user records a new expense to check for duplicates.`,
  ...
  execute: async (input, ctx) => {
    ...
    console.log(`[Insights] 🔄 check-duplicate-purchase: ${input.items.map((i) => i.name).join(', ')}`);
    ...
    for (const item of input.items) {
      // Get search text
      const searchQuery = item.nameLocalized
        ? `${item.name} ${item.nameLocalized}`
        : item.name;

      // Detect item type and get freshness window
      ...
      const result = await searchSimilarItems(
        searchQuery,
        sourceChannelId,
        freshnessWindow // Use freshness window as lookback
      );
      ...
      if (result.found && result.lastPurchase) {
        ...
        duplicates.push({
          ...
          message: isWithinWindow
            ? `You bought ${result.lastPurchase.name} ${result.lastPurchase.daysSince} days ago (${result.lastPurchase.quantity} ${result.lastPurchase.unit || 'units'}). Still have some?`
            : `Last purchase was ${result.lastPurchase.daysSince} days ago (outside ${freshnessWindow}-day window)`,
        });
      }
    }

    // Filter to only within-window duplicates for advisory
    ...
    if (relevantDuplicates.length > 0) {
      if (relevantDuplicates.length === 1) {
        ...
        advisoryMessage = `⚠️ Heads up! You recently bought:\n` +
          relevantDuplicates
            .map((d) => `• ${d.lastPurchase.name} (${d.lastPurchase.daysSince} days ago)`)
            .join('\n');
      }
    }
