...
/**
 * Split target for bill splitting
 */
...
/**
 * Create Expense Tool
 * Records a new expense with optional items and splits
 */
...
For receipts: pass receiptData from extract-receipt to link the receipt.`,
  inputSchema: z.object({
    ...
    category: z.enum(['Food', 'Transport', 'Groceries', 'Utilities', 'Entertainment', 'Shopping', 'Health', 'Education', 'Travel', 'Housing', 'Personal', 'Gift', 'Other']).optional()
      .describe('Expense category - auto-detected if not provided'),
    ...
    splitType: z.enum(['equal', 'exact', 'percentage', 'item']).optional()
      .describe('How to split: equal (divide equally), exact (specific amounts), percentage, item (by item assignment)'),
  ...
  execute: async (input, ctx) => {
    // Get context from input or RequestContext (auto-injected by router)
    const reqCtx = ctx?.requestContext;
    const channel = input.channel || reqCtx?.get('channel') as 'LINE' | 'WHATSAPP' | 'TELEGRAM';
    const senderChannelId = input.senderChannelId || reqCtx?.get('senderChannelId') as string;
    const sourceChannelId = input.sourceChannelId || reqCtx?.get('sourceChannelId') as string;
    const sourceType = input.sourceType || (reqCtx?.get('isGroup') ? 'GROUP' : 'DM');

    console.log(`\n${'='.repeat(60)}`);
    console.log(`[TOOL] 🔧 create-expense CALLED`);
    ...
    const categoryData = CATEGORIES[category as CategoryName];
    ...
    try {
      ...
      if (expenseDate) {
        ...
        if (!isToday) {
          // Use ISO date format - agent will localize in response
      ...
      if (input.items?.length) {
        ...
        for (const item of input.items) {
          ...
          // Keep item names as-is (original language) for querying
      ...
      if (splitInfo.length > 0) {
        message += '\n' + splitInfo.map(s =>
          `   → @${s.name} owes ${formatAmount(s.amount, response.expense.currency)}`
        ).join('\n');
      }
 ...
 * Query a specific expense by its ID (from quoted message EX:xxx)
 */
export const getExpenseByIdTool = createTool({
  ...
  execute: async (input) => {
    ...
    try {
      ...
      }>('GET', `/expenses/${input.expenseId}`, context);
...
export const deleteExpenseTool = createTool({
  ...
  execute: async (input) => {
    ...
    try {
      await apiRequest<{ success: boolean }>('DELETE', `/expenses/${input.expenseId}`, context);
